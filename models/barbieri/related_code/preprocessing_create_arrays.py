...
import os #aggiunta ema per richiamare gli id comuni
import pandas as pd
import numpy as np
import pickle
...
if __name__ == '__main__':
  ...
  icu_pat = icu_pat.sort_values('INTIME').drop_duplicates(subset='SUBJECT_ID', keep='first') #To select only the first ICU stay for each patient
  #Added to select only common patients
  common_ids_path = os.path.join(hp.data_dir, 'common_subject_ids.csv')
  common_ids_df = pd.read_csv(common_ids_path, header=0)
  common_ids = common_ids_df['SUBJECT_ID']
  
  icu_pat = icu_pat[icu_pat['SUBJECT_ID'].isin(common_ids)].reset_index(drop=True)
  #End of addition
  ...
  patients = np.sort(icu_pat['SUBJECT_ID'].values) 

  
  #train = patients.iloc[:29835]
  #test = patients.iloc[29835:33150]
  #validate = patients.iloc[26521:29836]
  train = patients[:26227]
  test = patients[26227:]
  validate = patients[22950:26228]
  ...
  sample_id = icu_pat['SUBJECT_ID'] #Added to include subject ID for identifying common patients to be tested
  print('Get patients corresponding to test ids')
  test_ids_patients = icu_pat['SUBJECT_ID'].iloc[test_ids].reset_index(drop=True)
  
  # -- Added for saving train/test split common subjects (to be used to select common patients in Agmon et al. model) --

  split_labels = np.where(icu_pat['SUBJECT_ID'].isin(test), 'test', 'train')
  df_split = pd.DataFrame({'SUBJECT_ID':icu_pat['SUBJECT_ID'], 'ASSIGNMENT':split_labels})
  df_split.to_csv('id_train_test.csv', index = False)
  print("Save id_train_test.csv")
  ...
  np.savez(hp.data_dir + 'data_arrays.npz', static=static, static_vars=static_vars, label=label,
           ...
           train_ids=train_ids, validate_ids=validate_ids, test_ids=test_ids, sample_id=sample_id)
